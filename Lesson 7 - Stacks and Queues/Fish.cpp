...
then fish P is initially upstream of fish Q. Initially, each fish has a unique 
position.
...
int solution(std::vector<int> &A, std::vector<int> &B){
	// Initialization
	int size = A.size();
	std::stack<int> downFishes;
	std::stack<int> upFishes;
	...
	for (int i = 0; i < size; ++i)
	{
		...
		if (B[i] == 0 && downFishes.size() != 0)
		{
			while (!downFishes.empty() && A[i] > downFishes.top()){
				downFishes.pop();
			}
			if (downFishes.empty()){
				upFishes.push(A[i]);
		...
		else if (downFishes.empty() && B[i] == 0 ){
			upFishes.push(A[i]);
		}
	}
